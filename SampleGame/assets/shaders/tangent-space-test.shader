...
void main()
{
    ...
    vec4 baseColor = texture(u_BaseTex, v_UV);
    vec4 stencilColor = texture(u_Stencil, v_UV);

    float stencilPass = mix(0.0, 1.0, stencilColor.w > 0.1);

    vec4 rgb = baseColor + gradientColor;
    rgb = rgb * stencilPass;

    color = vec4(rgb.x, rgb.y, rgb.z, stencilPass);
}
