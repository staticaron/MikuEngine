...
namespace MikuEngine
{
	void AssetLoader::Init()
	{
		...
		AddWatchDirectories( projectDir );
	...
	void AssetLoader::Update()
	{
		...
		while ( readBytes < rd )
		{
			...
			if ( event->mask & IN_CREATE ) m_CreateEvents.push_back( event );
			if ( event->mask & IN_MODIFY ) m_ModifyEvents.push_back( event );
			if ( event->mask & IN_MOVED_FROM ) m_MovedFromEvents.push_back( event );
			if ( event->mask & IN_MOVED_TO ) m_MovedToEvents.push_back( event );
			if ( event->mask & IN_DELETE ) m_DeleteEvents.push_back( event );
	...
	void AssetLoader::Destroy()
	{
		...
		close( m_Fd );
		MIKU_CORE_DEBUG( "Removing Watch From Asset Directory!" );
	}

	void AssetLoader::AddWatchDirectories( const std::string& directoryPath )
	{
